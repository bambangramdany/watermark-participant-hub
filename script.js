...
let events =
  JSON.parse(localStorage.getItem("wph_events")) ||
  defaultEvents;
...
let checkins =
  JSON.parse(localStorage.getItem("wph_checkins")) || [];
...
const openEventForm =
  document.getElementById("openEventForm");

const closeEventForm =
  document.getElementById("closeEventForm");
...
function getInvitationUrl(participantId) {
  return `${getBaseUrl()}invitation.html?id=${participantId}`;
}
...
function copyConfirmationLink(participantId) {
  ...
  showToast("Confirmation copied!");
}

function copyCheckinLink(participantId) {
  navigator.clipboard.writeText(
    getCheckinUrl(participantId)
  );

  showToast("Check-in copied!");
}

function copyInvitationLink(participantId) {
  navigator.clipboard.writeText(
    getInvitationUrl(participantId)
  );
...
function renderDashboard() {
  ...
  const arrived =
    participants.filter(
      p => getCheckinByParticipantId(p.id)
    ).length;
...
function renderNoShowList() {
  const list =
    document.getElementById("noShowList");

  list.innerHTML = "";

  const notArrivedParticipants =
    participants.filter(
      p => !getCheckinByParticipantId(p.id)
    );
...
participantForm.addEventListener(
  ...
  function (e) {
    ...
    const newParticipant = {
      ...
      tableNumber:
        document.getElementById(
          "participantTableNumber"
        ).value,
      mealNotes:
        document.getElementById(
          "participantMealNotes"
        ).value,
      status:
        document.getElementById(
          "participantStatus"
